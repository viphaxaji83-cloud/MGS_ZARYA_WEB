    ...
    <div style={{ padding: '24px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <Card style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', fontSize: 'var(--text-sm)' }}>
          ...
          <div>
            <div style={{ fontSize: '11px', textTransform: 'uppercase', opacity: 0.5, marginBottom: '4px' }}>Роль</div>
            <StatusBadge status={user?.role || 'operator'} label={user?.role === 'admin' ? 'Администратор' : 'Оператор'} size="md" />
          </div>
        </div>
      </Card>
      ...
      <Card style={{ marginBottom: '20px' }}>
        ...
        <form onSubmit={e => { e.preventDefault(); addToast('Функция смены пароля будет доступна в следующей версии', 'info'); }}
          ...
          {autoRefresh && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <input type="number" value={refreshInterval} onChange={e => setRefreshInterval(Number(e.target.value))}
                min={10} max={300} style={{
                  width: '80px', padding: '6px 10px', border: 'var(--border)', borderRadius: 'var(--radius-sm)',
